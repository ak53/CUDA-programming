...
__global__ void trans(int* mat, int* transmat, size_t pitch){
...
int main(){
	...
	// status = cudaMemcpy2D(mat, size*sizeof(int)/*no pitch on host*/,
 //             dmat, pitch/*CUDA pitch*/,
 //             size*sizeof(int)/*width in bytes*/, size/*heigth*/, 
 //             cudaMemcpyHostToDevice);
	status = cudaMemcpy2D(dmat, pitch/*no pitch on host*/,
             mat, size*sizeof(int)/*CUDA pitch*/,
             size*sizeof(int)/*width in bytes*/, size/*heigth*/, 
             cudaMemcpyHostToDevice); 
    if (status!=cudaSuccess){
	...
	trans<<<1,1>>>(dmat, dtransmat, pitch);
